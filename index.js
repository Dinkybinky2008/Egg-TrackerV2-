...
client.on('interactionCreate', async interaction => {
    try {
        ...
        // Defer reply for all commands to avoid InteractionNotReplied
        ...
        if (interaction.commandName === 'setup') {
            ...
            if (!hasAdmin) {
                await interaction.editReply('You must be an Administrator to run /setup');
                return;
            }
    ...
    if (body.embeds && body.embeds.length > 0) {
        ...
        if (e.fields && e.fields.length) {
            for (const f of e.fields) {
                ...
                if (name.includes('hatched from') || name.includes('from')) eggName = val.replace(/egg/ig,'').trim();
